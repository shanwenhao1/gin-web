...
# 安装依赖
RUN apt-get update \
    && apt-get -y install libcurl4-openssl-dev libapr1-dev libaprutil1-dev libmxml-dev \
    && echo -e "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$HOME/oss-server/kw_media/lib:/usr/local/lib\n\:\
export C_INCLUDE_PATH=$C_INCLUDE_PATH:/usr/include/apr-1.0">>$HOME/.bashrc && source $HOME/.bashrc \
    && apt-get -y install wget git tar cmake gcc g++ vim net-tools lrzsz \
